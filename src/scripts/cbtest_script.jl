...
trnset = task_type.(readlines("../data/FewGLUE/$task_name/train.jsonl"));
unlblset = task_type.(readlines("../data/FewGLUE/$task_name/unlabeled.jsonl"));
evalst = task_type.(readlines("../data/SuperGLUE/$task_name/val.jsonl"));
metrics = ["acc"]

pet_model_cnfg = WrapperConfig(albcnf, "xxlargev2/pytorch_model.bin", "mlm", lowercase(task_name), 256, label_list, 1)

sc_model_cnfg = WrapperConfig(albcnf, "xxlargev2/pytorch_model.bin", "sc", lowercase(task_name), 256, label_list, 1)
...
train_pet(
    ...
    output_dir="outputs/cb/ftullrepproperbias",
