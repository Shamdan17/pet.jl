...
trnset = flatten(task_type.(readlines("../data/mysteries-dataset/trncln.jsonl")));
unlblset = flatten(task_type.(readlines("../data/mysteries-dataset/vldcln.jsonl")));
evalst = flatten(task_type.(readlines("../data/mysteries-dataset/tstcln.jsonl")));
metrics = ["acc"]
...
train_pet(
    ...
    pattern_ids=[1, 2, 3], 
    output_dir="outputs/mysteries/attempt1",
