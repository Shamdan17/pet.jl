...
pet_train_cnf = TrainConfig(weight_decay=0.01,learning_rate=1e-5, num_train_epochs=3,batch_size=1,gradient_accumulation_steps=16, temperature=2, max_steps=250)
pet_eval_cnf = EvalConfig(metrics=metrics, batch_size=1)


sc_train_cnf = TrainConfig(weight_decay=0.01,learning_rate=1e-5, use_logits=true, num_train_epochs=1, max_steps=5000,gradient_accumulation_steps=16, temperature=2, batch_size=2)
...
train_pet(
    ...
    pattern_ids=[1], 
    output_dir="outputs/record/fastrep",
